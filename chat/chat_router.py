...
def _repair_json(text):
    """尝试修复 LLM 输出的不合法 JSON：成功返回修复后的字符串，失败返回 None。"""
    if not text:
        return None
    # 1) 优先用 json_repair 库
    try:
        ...
        if fixed and json.loads(fixed):
            return fixed
    except Exception:
        pass
    # 2) 规则兜底：修复“数组/对象结束后多一个引号”这类常见瑕疵
    try:
...
def _sse_json_response(payload: str, sid: str):
    """把一次性结果包装成 SSE 流返回，保证前端只有一种解析方式。"""
    async def gen():
        yield f"data:{json.dumps({'type': 'answer', 'content': payload, 'done': False}, ensure_ascii=False)}\n\n"
        yield f"data:{json.dumps({'type': 'answer', 'content': '', 'done': True, 'session_id': sid}, ensure_ascii=False)}\n\n"
...
def extract_json_from_text(text):
    ...
    # 5. 兜底：用 json_repair 修复 LLM 输出的不合法 JSON
    repaired = _repair_json(text)
    if repaired:
        logger.info("[JSON修复] 已自动修复不合法 JSON")
        return repaired
...
async def chat(request: Request, question: str = Form(...), user_id: str = Form(...), session_id: str = Form(None)):
    ...
    if intent == "chart":
        ...
        return _sse_json_response(extracted, sid)
