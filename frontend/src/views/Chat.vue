...
function renderChart(msgId, chartOption, retry = 0) {
  nextTick(() => {
    const el = document.getElementById('chart-' + msgId)
    if (!el || !chartOption) {
      if (retry < 8) setTimeout(() => renderChart(msgId, chartOption, retry + 1), 120)
      return
    }
    ...
    if (chartInstances[msgId]) { chartInstances[msgId].dispose(); delete chartInstances[msgId] }
    const chart = echarts.init(el)
    try { chart.setOption(chartOption, true) } catch (e) { console.error('ECharts error:', e) }
    chartInstances[msgId] = chart
    // 初始化后主动 resize 一次，防止尺寸计算为 0
